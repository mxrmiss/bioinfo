#!/usr/bin/env Rscript
# ==============================================================================
# Script Name: viz_enrichment_bubble.R (v2.4 Title Control Edition)
# Description: 
...
cat("================================================================\n")
cat(" Magic Bubble Plotter v2.4 (Title Control)\n")
cat("================================================================\n")
...
for (f_path in files) {
  ...
  # [NEW] Title Logic: Replace underscore with space, or set to NULL
  if (SHOW_PLOT_TITLE) {
    plot_title_text <- gsub("_", " ", f_base) # "GO_BP_up" -> "GO BP up"
  } else {
  ...
  p <- ggplot(df_ready, aes(x = GeneRatioNumeric, y = TermWrapped)) +
    ...
    labs(x = "GeneRatio", y = NULL, title = plot_title_text) +
  ...
  # --- Step 7: Save ---
  tryCatch({
    ggsave(file.path(OUT_DIR, paste0(f_base, ".bubble.pdf")), 
           plot = p, width = final_width_inch, height = final_height_inch, device = cairo_pdf)
    ggsave(file.path(OUT_DIR, paste0(f_base, ".bubble.png")), 
